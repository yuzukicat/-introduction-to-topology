  ...
  \begin{columns}
    \begin{column}{0.6\textwidth}
      \begin{block}{}
        ...
        \begin{itemize}
        \item It actually tells us that \textsl{P} is made up of two discs which are identified along their boundaries.
...
\begin{frame}{Topological equivalence}
  \begin{columns}
    ...
    \begin{column}{0.6\textwidth}
      \begin{block}{}
        \begin{itemize}
        \item The polyhedron that is not convex and does not lend itself to the above argument.
...
\begin{frame}{Topological equivalence}
  \begin{block}{}
    We propose to give a specific homeomorphism (i.e., a continuous, one-one, and onto function which has continuous inverse) from space (c) to space (d).
  \end{block}
  \begin{columns}
    \begin{column}{0.5\textwidth}
      \begin{figure}
...
\begin{frame}{Topological equivalence}
  \begin{block}{}
    \begin{itemize}
    \item A way of defining a homeomorphism from the polyhedron \textsl{P} to the sphere.
    \item It is possible to produce an argument in the opposite direction and show that if \textsl{P} is topologically equivalent to the sphere then \textsl{P} satisfies hypotheses (a) and (b) of Euler's theorem, and therefore Euler's theorem holds for \textsl{P}.
    \item So if \textsl{P} and \textsl{Q} are polyhedra which are both homeomorphic to the sphere, and if we call $v - e + f$ the \textsl{Euler number} of a polyhedron, then we know from the above discussion that \textsl{P} and \textsl{Q} have the same Euler number, namely 2.
    \end{itemize}
  \end{block}
\end{frame}
...
\begin{frame}{Topological equivalence}
  % Theorem environment
  \begin{theorem}[(1.2) Topologically equivalent polyhedra have the same Euler number.]
  \end{theorem}
\end{frame}
...
\begin{frame}{Surfaces}
  \begin{block}{}
    ...
    \item In what follows we shall try to translate the notion of a surface into precise mathematical language.
    \item Involving firstly the definition of an abstract (topological) space, and secondly the recognition of surfaces as those spaces which look locally like the euclidean plane.
    \end{itemize}
  \end{block}
\end{frame}
